...
video=B/'output/Mochi_80秒_2K120帧_V5.mp4'
Q=B/'evidence/v5-final-qa';Q.mkdir(exist_ok=True,parents=True)
...
assert abs(float(probe['format']['duration'])-80)<.04
assert int(v['nb_frames'])==9600
...
subprocess.run(['ffmpeg','-v','error','-y','-i',str(video),'-vf','fps=1/2,scale=640:360,tile=3x5','-frames:v','3',str(Q/'contact-%02d.jpg')],check=True)
for t in [1.25,1.45,9.8,15.5,18.7,20.7,22.9,25,28.9,30.2,31.2,32.5,35,40,47.3,53.5,59,64.5,68,72.5,78,78.3]:
