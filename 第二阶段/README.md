...
> 🔴 **时间已提前（2026-09-08 用户裁定）：新截止 2026-09-11，唯一事实源 =
> `docs/tasks/MOCHI-URGENT-REPLAN-2026-09-11.md`。**
> **记忆系统代码层已完成度：6 工具插件（note/recall/forget/world/list/clear），已注册三 profile 并挂载，
> 监制复跑全绿 —— 本阶段"落地两步"实质已兑现，不再占本轮工期。**
> ⚫ 本轮砍：记忆 UI 设置页（对话内 list/forget/clear 已够用）、会话开始自动注入契约（TO_BE_RESOLVED，答辩后）。
> 打包侧提醒：`mochi-memory` 必须进白名单（URGENT-01），否则打包版缺插件。
