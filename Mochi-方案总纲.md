...
> ⛔ **本文件已作废（superseded）· 2026-09-12 裁定**
>
> **status**: superseded　**last_verified**: 2026-09-12　**verified_by**: 工具线
>
> **替代者 = `Mochi-总体方案.md`（v2.0，L1 产品设计事实源）。**
> 本文是 2026-09-04 立项期的方案总纲，其排期、内核版本、插件范围均已被后续裁定推翻。
> **不得作为实现依据**；正文原样保留，仅供追溯。参见 `docs/DOC-AUTHORITY.md` §4。
>
> 已知过时点：① 头部「截止 2026-09-30」指的是**嘉行联参赛材料**，Mochi 出包已于 2026-09-11 交付；
> ② 内核版本写作 `0.1.2-rc.1`，真值为 `0.1.3-alpha.1`（`prepare-mochi-resources.cjs` 的 `PLUGIN_RUNTIME_VERSIONS`）；
> ③ 插件范围、打包口径以 `docs/build-standard.md` 为准。

> 赛事：嘉祥教育集团 AI 创意实践大赛（高中组）　立项 2026-09-04
> 内核：Mochi Harness（dsh）`0.1.3-alpha.1`　MIT 协议　Everything is a Plugin
...
> 🔴 **本表已作废（2026-09-08 用户裁定：外部把时间提前了）。**
> **新截止 2026-09-11（周五），唯一事实源 = `docs/tasks/MOCHI-URGENT-REPLAN-2026-09-11.md`。**
> 下方为原始四周排期，仅作历史留档，不得再据此排期。
...
**压缩后的实际排期（9/9–9/11，三天）**

| 日期 | 目标 | 止损线 |
|---|---|---|
| 9/9 周三 | 校园网可达性实测 + 打包白名单 12→16 + cwd/env 修复 + 出 dmg | 打不出 dmg → 退 dir+.zip；白名单卡住 → 只保三个工具插件 |
| 9/10 周四 | 全新机双击验收 + 演示路径彩排 + **演示录屏必须出片** | 任一阻断场景挂 → 切录屏演示 |
| 9/11 周五 | 答辩演练（嘉行联 + Mochi）+ 材料齐套 → 交付 | 现场包 = dmg + 录屏 + 本地兜底三件套 |
