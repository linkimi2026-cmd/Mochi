...
> ⚠️ **状态更新（2026-09-12 补加，正文一字未删）**
>
> 原文头部写「**状态：待批准**」「日期：2026-09-06」——**均已过时**：
> - 本方案涉及的工作**已批准并已实施**，桌面端双平台安装包已于 2026-09-11 交付（`release/2026-09-09`、`release/2026-09-10`）。
> - 正文 §21.3 写「现有白名单只覆盖 **9 个插件**」，该数字**已过时**：当前白名单为 **26 个插件**（`apps/desktop/scripts/prepare-mochi-resources.cjs` 的 `PLUGINS`）。
> - 本文属**设计阶段文档**（L4 历史档案），与源码冲突时**以源码为准**（`docs/DOC-AUTHORITY.md` L2）。
