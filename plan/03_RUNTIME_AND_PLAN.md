...
>
> **替代者 = `foundation/` 包 → 再被 `Mochi-总体方案.md` v2.0 取代。**
> 本文是 Codex 落地前的早期运行时规划，其内核版本、`TO_BE_RESOLVED_BY_CODEX` 待办、
> 工具族命名（`ppt.*/doc.*/file.*/browser.*` 点号形态）**均已作废**。
> 「工具名只允许 `^[a-zA-Z0-9_-]+$`，禁止点号」——点号会让模型网关整轮 400 拒收。
> 不得作为实现依据；正文原样保留，仅供追溯。参见 `docs/DOC-AUTHORITY.md` §4。

> 作者角色：运行时 / 质量体系架构师　|　面向：教师办公 AI Work Agent「Mochi」桌面 APP
> 内核事实底座：Mochi Harness `0.1.3-alpha.1`（现行真值）；源码 `mochi-harness-src.nosync/mochi-harness-master/`
> 历史文件内保留的 `0.1.2-rc.1` 为当时（2026-09-05 前后）的 npm 实测版本，属过时口径。
...
> 🔴 **本节的"26 天"人日估算已作废（2026-09-08 用户裁定：外部把时间提前了）。**
> **新截止 2026-09-11，只剩 3 天（9/9–9/11）。唯一事实源 = `docs/tasks/MOCHI-URGENT-REPLAN-2026-09-11.md`。**
> 人日数字保留作历史参考；**P1/P2 全部降级为"答辩后"**，P0 内部再按 URGENT 编号裁剪（见紧急表 §4 砍项清单）。
...
**现行（3 天）**：只保 P0 中"能演示"的最小集 —— 能双击打开 → 能对话 → 能跑通传话/PPT 二次编辑/成绩分析/教学建模 → 有录屏兜底。
Eval 150 条 case、Win 打包、A2A 单机演示均移出本轮。
