...
>
> 本文是 Codex 落地前的早期设计修订日志，**不得作为实现依据**。正文原样保留，仅供追溯。
> **P0-7「dsh 未锁版本」的最终裁决（2026-09-12）**：真值 = **`0.1.3-alpha.1`**，
> 来源 `apps/desktop/scripts/prepare-mochi-resources.cjs` 的 `PLUGIN_RUNTIME_VERSIONS`
> （全部条目均为 `0.1.3-alpha.1`）。文中保留的 `0.1.2-rc.1` 是当时（2026-09-05 前后）
> 的 npm 实测版本，属**过时的历史口径**，不删改以保真。
> 参见 `docs/DOC-AUTHORITY.md` §6。
...
| P0-7 dsh 未锁版本 | ✅ 已解决（当时锁 `0.1.2-rc.1`；**2026-09-12 终裁真值 = `0.1.3-alpha.1`**，见头部横幅） | 03 §8.2 |
