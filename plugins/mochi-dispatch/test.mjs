...
// Relay reports this timestamp form without an offset. Keep normal pending
// fixtures relative to the test run; explicit expiry cases below use a past
// timestamp and remain independent assertions.
const pendingRelayCreatedAt = new Date().toISOString().replace('T', ' ').replace(/\.\d{3}Z$/u, '');
...
console.log('④ mochi_ask：审批→建任务→relay 投递→DELIVERED+关联');
{
  const { tools, conn, exec, store } = makeHarness();
  const res = await tools.get('mochi_ask').execute({ peerName: '张老师', goal: '周五第三节能不能换课' }, exec);
...
console.log('⑤ mochi_ask：同参数重放→拦截（不重复投递）');
{
  const { tools, conn, exec } = makeHarness();
  const a = await tools.get('mochi_ask').execute({ peerName: '张老师', goal: '换课' }, exec);
  const b = await tools.get('mochi_ask').execute({ peerName: '张老师', goal: '换课' }, exec);
...
console.log('⑥ 审批拒绝→【未派发】如实报错');
{
  ...
  await assert.rejects(() => tools.get('mochi_ask').execute({ peerName: '张老师', goal: '换课' }, exec), /未派发/);
}

console.log('⑦ mochi_find：登记命中→不建任务；未命中→委托+request 投递');
{
  const { tools, conn, exec } = makeHarness();
  ...
  const hit = await tools.get('mochi_find').execute({ item: '急救包' }, exec);
  assert.equal(hit.登记命中, true);
  const { tools: t2, conn: c2, exec: e2 } = makeHarness();
  const miss = await t2.get('mochi_find').execute({ item: '期中数学卷子', peerName: '周医生' }, e2);
...
console.log('⑦a relay 关联：只接受本次确定 ID，不从返回列表最大 ID 猜测');
{
  ...
  const requestTask = await tools.get('mochi_request').execute({ peerName: '张老师', goal: '请留意设备' }, exec);
  assert.equal(requestTask.relayMessageId, 301);
  assert.equal(store.getTask(requestTask.taskId).relay_message_id, 301);
  const findTask = await tools.get('mochi_find').execute({ item: '借书卡', peerName: '周医生' }, exec);
...
console.log('⑦b relay 关联：缺失或无效 ID 保持结果不明，不自动重发');
{
  ...
  await assert.rejects(() => tools.get('mochi_ask').execute({ peerName: '张老师', goal: '关联缺失' }, exec), /结果不明/);
  ...
  const duplicate = await tools.get('mochi_ask').execute({ peerName: '张老师', goal: '关联缺失' }, exec);
  assert.equal(duplicate.重复投递已拦截, true);
  assert.equal(sends, 1);
  await assert.rejects(() => tools.get('mochi_ask').execute({ peerName: '张老师', goal: '关联缺失', retryTaskId: pending.id }, exec), /没有可确认的未投递失败/);
  const forcedNew = await tools.get('mochi_ask').execute({ peerName: '张老师', goal: '关联缺失', newTask: true }, exec);
  ...
  await assert.rejects(() => invalidTools.get('mochi_ask').execute({ peerName: '李老师', goal: '关联无效' }, invalidExec), /结果不明/);
...
console.log('⑦c relay 重试：只接受结构化、路径与状态均匹配的未投递失败；新任务独立关联');
{
  ...
  await assert.rejects(() => tools.get('mochi_ask').execute(args, exec), /未投递/);
  ...
  const ordinaryReplay = await tools.get('mochi_ask').execute(args, exec);
  ...
  const [firstRetry, secondRetry] = await Promise.all([
    tools.get('mochi_ask').execute({ ...args, retryTaskId: failed.id }, exec),
    tools.get('mochi_ask').execute({ ...args, retryTaskId: failed.id }, otherSession),
  ]);
  ...
  for (let index = 0; index < 3; index += 1) {
    const newTask = await tools.get('mochi_ask').execute({ ...args, newTask: true }, exec);
...
console.log('⑦d relay 重试：错误文案、路径或状态任一不匹配仍是 UNKNOWN');
{
  ...
  for (const [index, unsafeFailure] of unsafeFailures.entries()) {
    ...
    await assert.rejects(() => tools.get('mochi_ask').execute({ peerName: '张老师', goal: `不安全失败 ${index}` }, exec), /结果不明/);
...
console.log('⑦e relay 写入前：DISPATCHING 已持久化为 UNKNOWN，崩溃窗口不留下 PENDING');
{
  ...
  const result = await tools.get('mochi_ask').execute({ peerName: '张老师', goal: '确认写入前状态' }, exec);
...
console.log('⑦f relay 中断恢复：UNKNOWN 过期后仍不能借 newTask 二次发送');
{
  ...
  const blocked = await tools.get('mochi_ask').execute({ peerName: '张老师', goal: '中断后的同一任务', newTask: true }, { agent: { session: {} }, callId: 'interrupted-new-task' });
...
console.log('⑧ mochi_tasks：同步 relay 状态（accepted→COMPLETED 带回话）+ 入站镜像');
{
  const { tools, conn, exec } = makeHarness();
  const res = await tools.get('mochi_ask').execute({ peerName: '张老师', goal: '换课' }, exec);
  conn.rows[0].status = 'accepted';
  conn.rows[0].reply = '可以，周五见';
  const list = await tools.get('mochi_tasks').execute({}, exec);
  ...
  conn.rows.push({ id: 201, direction: 'incoming', peerName: '周宁老师', peerRole: '校医', kind: 'message', body: '周宁老师的 Mochi 替主人带话：【问询】明天能借用一次教室吗', status: 'pending', reply: '', createdAt: pendingRelayCreatedAt });
  const list2 = await tools.get('mochi_tasks').execute({}, exec);
...
console.log('⑨ mochi_respond：入站任务应答 approve→COMPLETED；出站任务拒绝回应');
{
  const { tools, conn, exec } = makeHarness();
  const res = await tools.get('mochi_ask').execute({ peerName: '张老师', goal: '换课' }, exec);
  conn.rows.push({ id: 202, direction: 'incoming', peerName: '周宁老师', peerRole: '校医', kind: 'message', body: '借用教室', status: 'pending', reply: '', createdAt: pendingRelayCreatedAt });
  const list = await tools.get('mochi_tasks').execute({}, exec);
  const inboundId = list['等我回应'][0].taskId;
  const done = await tools.get('mochi_respond').execute({ taskId: inboundId, decision: 'approve', note: '可以，明天说' }, exec);
  ...
  await assert.rejects(() => tools.get('mochi_respond').execute({ taskId: res.taskId, decision: 'approve' }, exec), /不是收到来的任务/);
  // 重复应答 → 终态不可逆
  await assert.rejects(() => tools.get('mochi_respond').execute({ taskId: inboundId, decision: 'decline' }, exec), /终态/);
}
...
console.log('⑪ 归属与幂等迁移：旧出站恢复 canonical fingerprint，未知旧字段保守阻断');
{
  ...
  assert.ok(columns.includes('receipt_seen_at'));
  ...
  const sameAsk = await tools.get('mochi_ask').execute({ peerName: '周宁老师', goal: '历史出站任务' }, legacyExec);
  const sameRequest = await tools.get('mochi_request').execute({ peerName: '教导主任', goal: '旧版请求任务', context: '旧背景' }, legacyExec);
  const sameFind = await tools.get('mochi_find').execute({ item: '急救包', peerName: '周医生' }, legacyExec);
  const uncertainFind = await tools.get('mochi_find').execute({ item: '校牌', peerName: ' 赵老师 ' }, legacyExec);
  ...
  const otherPeerFind = await tools.get('mochi_find').execute({ item: '校牌', peerName: ' 李老师 ' }, legacyExec);
...
console.log('⑫ 同名账号：按稳定 owner 隔离入站镜像，非 owner 不触发审批或 transport');
{
        ...
        return {
          ...
          result: {
            ...
            incoming: [{
              ...
              status: 'pending',
              reply: '',
              createdAt: pendingRelayCreatedAt,
            }],
          },
        };
      }
  ...
  const listA = await tools.get('mochi_tasks').execute({}, execA);
  const listB = await tools.get('mochi_tasks').execute({}, execB);
  ...
  await assert.rejects(() => tools.get('mochi_respond').execute({ taskId: taskA, decision: 'approve' }, execB), /不属于当前校园账号/);
  ...
  const completed = await tools.get('mochi_respond').execute({ taskId: taskA, decision: 'approve', note: '我来处理' }, execA);
