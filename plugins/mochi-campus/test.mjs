...
// ① 注册面：全部工具在册
{
  ...
  assert.deepEqual(
    ...
    [
      ...
      'jxl_event_detail',
      'jxl_health_events',
      'jxl_medical_event_create',
      'jxl_medical_event_status',
      'jxl_message',
      'jxl_movement_detail',
      'jxl_movement_link_medical_event',
      'jxl_movement_request_create',
      'jxl_movement_request_decide',
      'jxl_movement_request_list',
      'jxl_movement_transition',
      'jxl_query',
      'jxl_relay_find',
      'jxl_relay_list',
      'jxl_relay_register',
...
// 学生目录与流动单是不同事实域：姓名解析必须查真实 students 路由。
{
  ...
  const { ctx, tools } = makeCtx('allowed-once');
  apply(ctx, makeConnection());
  await tools.get('jxl_student_directory_search').execute({ keyword: '简乐知', limit: 10 }, exec);
  assert.equal(calls[0].path, '/api/students?search=%E7%AE%80%E4%B9%90%E7%9F%A5&limit=10');
}

// ② jxl_relay_list 直连传话箱端点（POST /api/assistant/relay）
{
  calls.length = 0;
  const { apply } = await import('./index.mjs');
  const { ctx, tools } = makeCtx('allowed-once');
  apply(ctx, makeConnection());
  const result = await tools.get('jxl_relay_list').execute({}, exec);
  ...
  console.log('② jxl_relay_list 直连 + 摘要 OK');
}
...
// 新增业务写工具在没有官方 approval 服务时必须在任何上游写请求前失败。
{
  ...
  await assert.rejects(
    () => tools.get('jxl_movement_transition').execute({ reference: 'mov_demo', action: 'confirm-return', expectedVersion: 4, idempotencyKey: 'test-return-0001' }, exec),
    /没有人工确认通道/,
  );
...
// 医务写工具：角色、人工拒绝、学生一致性与 CAS 参数都必须在真实写入前守住。
{
  ...
  await assert.rejects(
    () => wrongRole.tools.get('jxl_medical_event_create').execute({ studentId: 15, category: '身体不适', urgency: '普通', measure: '休息观察', status: '留观中', note: '测试', idempotencyKey: 'event-create-0001' }, exec),
    /只有校医/,
  );
  ...
  await assert.rejects(
    () => rejected.tools.get('jxl_medical_event_create').execute({ studentId: 15, category: '身体不适', urgency: '普通', measure: '休息观察', status: '留观中', note: '测试', idempotencyKey: 'event-create-0002' }, exec),
  ...
  await assert.rejects(
    () => mismatch.tools.get('jxl_movement_link_medical_event').execute({ reference: 'mov_demo', medicalEventId: 301, expectedVersion: 2, idempotencyKey: 'event-link-0001' }, exec),
    /不同学生/,
  );
  ...
  await status.tools.get('jxl_medical_event_status').execute({ eventId: 301, status: '准备返班', expectedVersion: 4, idempotencyKey: 'event-status-0001' }, exec);
...
// ③ jxl_relay_send 审批闸：无审批通道 → 拒发；主人拒绝 → 拒发；放行 → 恰好一次投递
{
  {
    const { apply } = await import('./index.mjs');
    ...
    apply(ctx, makeConnection());
    await assert.rejects(
      () => tools.get('jxl_relay_send').execute({ peerName: '夏医生', note: '明早送急救包' }, exec),
      /没有人工确认通道/,
    );
  }
  {
  ...
  }
  {
    ...
    const first = await tools.get('jxl_relay_send').execute(payload, exec);
    const second = await tools.get('jxl_relay_send').execute(payload, exec);
  ...
  console.log('③ jxl_relay_send 审批闸 + 单次投递语义 OK');
}

// ④ jxl_relay_send 参数校验：寻物缺东西名、传话缺正文都不得进入审批
{
  const { apply } = await import('./index.mjs');
  const { ctx, tools } = makeCtx('allowed-once');
  apply(ctx, makeConnection());
  await assert.rejects(() => tools.get('jxl_relay_send').execute({ peerName: '夏医生', kind: 'request' }, exec), /东西/);
  await assert.rejects(() => tools.get('jxl_relay_send').execute({ peerName: '夏医生', kind: 'message' }, exec), /正文/);
  await assert.rejects(() => tools.get('jxl_relay_send').execute({ kind: 'message', note: 'x'.repeat(121) }, exec), /peerName/); // schema 先拦缺参
  await assert.rejects(() => tools.get('jxl_relay_send').execute({ peerName: '夏医生', kind: 'message', note: 'x'.repeat(121) }, exec), /120/);
  console.log('④ jxl_relay_send 参数校验 OK');
}

// ⑤ jxl_relay_respond：应答必须过审批（人决定应答的执行面）
{
  calls.length = 0;
  const { apply } = await import('./index.mjs');
  const { ctx, tools } = makeCtx('allowed-once');
  apply(ctx, makeConnection());
  await tools.get('jxl_relay_respond').execute({ id: 7, action: 'accept', note: '找到了，这就送去' }, exec);
  ...
  await assert.rejects(() => toolsNo.get('jxl_relay_respond').execute({ id: 7, action: 'accept' }, exec), /未发送|未登记|未回应|未获主人确认/);
  assert.equal(calls.filter((c) => c.path === '/api/assistant/relay/respond').length, 1, '被拒的应答不得触达上游');
  console.log('⑤ jxl_relay_respond 人决定应答 OK');
}

// ⑥ jxl_relay_find / jxl_relay_register 语义
{
  calls.length = 0;
  const { apply } = await import('./index.mjs');
  const { ctx, tools } = makeCtx('allowed-once');
  apply(ctx, makeConnection());
  await tools.get('jxl_relay_find').execute({ item: '期中卷子' }, exec);
  assert.equal(calls[0].path, '/api/assistant/relay/find');
  await assert.rejects(() => tools.get('jxl_relay_find').execute({}, exec), /item/); // schema 层拦缺参
  await tools.get('jxl_relay_register').execute({ item: '备用 basketball' }, exec);
  assert.equal(calls[1].path, '/api/assistant/relay/register');
  console.log('⑥ jxl_relay_find / jxl_relay_register OK');
}
...
//（2026-09-05 事故：单参 render 让模型收到的"结果"永远是 args，抓包 016/019 定位）
{
  ...
  const { ctx, tools } = makeCtx('allowed-once');
  apply(ctx, makeConnection());
  const list = tools.get('jxl_relay_list');
  ...
  const find = tools.get('jxl_relay_find');
