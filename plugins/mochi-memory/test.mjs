...
{
  const noted = await call('mochi_memory_note', {
  ...
  const recalled = await call('mochi_memory_recall', { query: '绿色粉笔' });
  ...
  const again = await call('mochi_memory_recall', { query: '绿色粉笔' });
  assert.equal(again.命中[0]['为什么Mochi知道这个'].includes('已被召回 2 次'), true, '召回计数要随命中递增');

  const forgotten = await call('mochi_memory_forget', { id: noted.记忆ID });
  assert.equal(forgotten.已忘记, true);
  assert.equal(forgotten.原内容.includes('绿色粉笔'), true, 'forget 要返回被忘快照');

  const after = await call('mochi_memory_recall', { query: '绿色粉笔' });
...
console.log('③ 敏感内容被护栏拒绝，错误话术以【未写入】开头');
{
  await assert.rejects(
    () => call('mochi_memory_note', { kind: 'task_fact', content: '主人的系统密码是 123456' }),
    (error) => error.message.startsWith('【未写入】'),
  );
  await assert.rejects(
    () => call('mochi_memory_note', { kind: 'task_fact', content: '张三的诊断结果是……' }),
    (error) => error.message.startsWith('【未写入】'),
  );
}
...
console.log('④ convention 同步进 world-state');
{
  ...
  const snapshot = await call('mochi_memory_world', { action: 'read' });
  ...
  await call('mochi_memory_forget', { id: noted.记忆ID });
...
{
  const read0 = await call('mochi_memory_world', { action: 'read' });
  assert.ok(read0.工作状态全文.includes('# Mochi 工作状态'), 'read 要返回骨架齐全的全文');

  const added = await call('mochi_memory_world', { action: 'append-todo', text: '明天把教案交给年级组' });
  assert.equal(added.已加入待办, '明天把教案交给年级组');
  let snapshot = await call('mochi_memory_world', { action: 'read' });
  assert.ok(snapshot.工作状态全文.includes('明天把教案交给年级组'));

  const done = await call('mochi_memory_world', { action: 'complete-todo', text: '明天把教案交给年级组' });
  assert.equal(done.已完成, true);
  snapshot = await call('mochi_memory_world', { action: 'read' });
  assert.ok(!snapshot.工作状态全文.includes('明天把教案交给年级组'), '完成后待办要移除');

  const miss = await call('mochi_memory_world', { action: 'complete-todo', text: '不存在的待办' });
...
console.log('⑦ note 带 source 透传，recall 的「为什么Mochi知道这个」显示「多次重复」');
{
  ...
  assert.equal(noted.已记住, true);
  const recalled = await call('mochi_memory_recall', { query: '红蓝双色笔' });
  assert.equal(recalled.命中条数, 1);
...
{
  const before = await call('mochi_memory_list', {});
  ...
  const noted = await call('mochi_memory_note', { kind: 'task_fact', content: '下周三要交期中试卷分析给年级组', summary: '期中试卷分析' });
  const listed = await call('mochi_memory_list', {});
  assert.equal(listed.统计.总数, before.统计.总数 + 1, '写入后总数要 +1');
  assert.ok(listed.记忆列表.some((row) => row.记忆ID === noted.记忆ID), '列表要含刚写入的条目');

  await call('mochi_memory_forget', { id: noted.记忆ID });
  const after = await call('mochi_memory_list', {});
...
console.log('⑨ memory_clear 链路：3 条（含 1 pinned）→ clear → 空库 → 审计有 forget 快照');
{
  ...
  await call('mochi_memory_clear', {}); // ⑦ 还留着 1 条「红蓝双色笔」，先归零再写入，保证删除条数=3 的断言精确。
  const ids = [];
  for (const content of ['主人习惯周五下午开班会', '年级组办公室的钥匙在门卫室登记', '主人批改用 A4 横格纸做草稿']) {
    const noted = await call('mochi_memory_note', { kind: 'task_fact', content });
    ids.push(noted.记忆ID);
  }
  direct.pin(ids[2], true); // 1 条 pinned 免疫记忆

  const cleared = await call('mochi_memory_clear', {});
...
{
  const again = await call('mochi_memory_clear', {});
