...
> **目标**：老师双击图标 → 直接可用。全程不出现浏览器、不出现终端、不需要任何配置。
> **状态**：**待批准**。本文只做诊断与方案设计，未改动任何代码，未执行打包。
> **日期**：2026-09-06
> **已拍板决策**：① 免签名 + 一键安装脚本 ② 内置默认密钥 + 设置页可改 ③ Mac 双架构 + Windows ④ 内置公共 SearXNG 端点

> 🔴 **时间已提前（2026-09-08 用户裁定）：新截止 2026-09-11（周五），本方案内部的实施顺序不再自带排期。**
>
> ⚠️ **状态更正（2026-09-12 补加，正文一字未删）**：头部「状态：待批准」已过时——本方案涉及的工作**已批准并已实施**，双平台安装包已于 2026-09-11 交付（release/2026-09-09、release/2026-09-10）。正文提到的「白名单只覆盖 9 个插件」亦已过时：当前白名单为 **26 个插件**（apps/desktop/scripts/prepare-mochi-resources.cjs 的 PLUGINS）。本文属历史档案，与源码冲突时**以源码为准**。
> **唯一事实源 = `docs/tasks/MOCHI-URGENT-REPLAN-2026-09-11.md`。**
> 本轮只做"双击可用"的最小路径：打包白名单 12→16（URGENT-01）+ cwd/env 修复（URGENT-02）+ 出 dmg 与双击验收（URGENT-03）。
> ⚫ 本轮砍：首启向导、环境自检/Mochi 医生、四项零依赖硬改、开机自启、Windows 首包（见紧急表 §4）。
...
| 目标 | 方案 | 说明 |
|---|---|---|
| macOS arm64 | **本机直接打** | 最快，先做这个 |
| macOS x64 | GitHub Actions `macos-13`（Intel runner）或借一台 Intel Mac | `sharp`/`node-pty` 需原生 rebuild |
| Windows x64 | GitHub Actions `windows-latest` 或借一台 Windows 机器 | `runtime-profile.cjs:169` 的 junction 分支、node-pty 都只在 Windows 走到，**从未验证过** |
