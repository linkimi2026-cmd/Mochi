...
// ---- 探测工作区选择器 ----
console.log('open ws menu:', await clickByText(page, '备课与课件'));
await page.waitForTimeout(2500);
await shot(page, 'M-01-ws-menu');
const menu = await page.evaluate(() => {
  const out = [];
  for (const el of document.querySelectorAll('*')) {
    const t = (el.textContent || '').trim();
    if (!t || t.length > 24 || el.children.length) continue;
    const r = el.getBoundingClientRect();
    if (r.width < 40 || r.height < 16 || r.height > 60 || r.y > 700) continue;
