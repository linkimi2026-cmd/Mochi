...
async function poll(page, marks, tag, maxMs = 120000, needStable = 1) {
...
await installHider(page);
await shot(page, 'N-01-campus');
console.log('返回 Mochi:', await clickByText(page, '返回 Mochi'));
await page.waitForTimeout(6000);
await installHider(page);

// ---- 切换工作区到「班级与教室」 ----
console.log('打开工作区菜单:', await clickByText(page, '备课与课件'));
await page.waitForTimeout(2500);
console.log('选择班级与教室:', await clickByText(page, '班级与教室'));
await page.waitForTimeout(4000);
await installHider(page);
await shot(page, 'N-02-workarea');
const wsTxt = await page.evaluate(() => document.body.innerText);
console.log('工作区确认:', wsTxt.slice(0, 300).replace(/\n+/g, ' | '));

// ---- 新会话 + A2A ----
console.log('新会话:', await clickByText(page, '新会话'));
await page.waitForTimeout(5000);
await installHider(page);
await typeAndSend(page, '用传话工具帮我给周医生带句话：今天下午三点我到医务室拿体温计，请等我。');
console.log('[sent]');

const t1 = await poll(page, ['等待审批', '允许一次', '确认发送', '确认把以上口信投递'], 'card-1', 130000, 1);
await shot(page, 'N-03-card1');
console.log('--- card1 ---\n' + t1.slice(-700));

if (t1.includes('确认发送') && !t1.includes('允许一次')) {
  console.log('确认发送:', await clickByText(page, '确认发送'));
  await page.waitForTimeout(5000);
  await poll(page, ['等待审批', '允许一次'], 'approval', 100000, 1);
}
...
for (let round = 1; round <= 3; round++) {
  ...
  if (isRelay) {
    await shot(page, 'N-04-a2a-approval');
    console.log('允许一次:', await clickByText(page, '允许一次'));
    await page.waitForTimeout(9000);
    await poll(page, ['已送达', '已投递', '投递成功', '已转达', '已完成'], 'done', 110000, 1);
    await shot(page, 'N-05-a2a-done');
    break;
  } else {
    console.log('允许一次(前置):', await clickByText(page, '允许一次'));
    await page.waitForTimeout(9000);
