...
await page.waitForTimeout(600);
await page.screenshot({ path: `${OUT}/h-00-campus.png` });
...
await page.mouse.move(800, 100);
await page.waitForTimeout(600);
await page.screenshot({ path: `${OUT}/h-01-back-to-chat.png` });
...
await hideWorkbench(page);
try {
  await ask(page, '帮我看看现在有没有学生外出超时还没返班，用工具查一下', ['工具调用', '返班', '林小禾'], 'h-02-student-query');
...
await hideWorkbench(page);
try {
  await ask(page, '用传话工具帮我给周医生带句话：今天下午三点我到医务室拿体温计，请等我。', ['工具调用', '审批', '允许一次', '传话', '周医生'], 'h-03-a2a-approval', 180000);
...
try {
  ...
  if (await btn.count()) {
    ...
    await page.mouse.move(800, 100);
    await page.waitForTimeout(700);
    await page.screenshot({ path: `${OUT}/h-04-approved.png` });
